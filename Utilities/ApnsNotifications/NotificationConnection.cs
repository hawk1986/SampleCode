...
namespace Utilities.ApnsNotifications
{
    ...
    public class NotificationConnection : IDisposable
    {
        ...
        private readonly NotificationChannel apnsChannel;
        #endregion
        ...
        {
            if (!isDisposing && !isClosing && isAccepting)
            {
        ...
        public void Close()
        {
            ...
            // would exit due to closing set true
        ...
        {
            while (!isDisposing && !isClosing)
            {
                try
                {
                    while (this.notifications.Count > 0 && !isDisposing)
                    {
